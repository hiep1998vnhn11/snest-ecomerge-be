...
`,
  ...
  const handleActionCategory = async (params: State) => {
    try {
      setIsLoading(true)
  ...
  const slugify = (str: string) => {
    const a =
      'ầệãảàáâäæãåāăąçćčđďẻèéẽêëēėęěğǵḧîïíīįìłḿñńǹňôöòóœøōõőṕŕřßśšşșťțûüùúūǘůűųẃẍÿýžźż·/_,:;'
    const b =
      'aeaaaaaaaaaaaacccddeeeeeeeeeegghiiiiiilmnnnnoooooooooprrsssssttuuuuuuuuuwxyyzzz------'
  ...
  const actualPageMarkup = (
    <Page title={(isCreate ? 'Thêm mới' : 'Sửa') + ' danh mục sản phẩm'}>
      <Layout>
        {skipToContentTarget}
